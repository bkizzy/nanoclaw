...
/**
 * Decide whether a `fromMe` inbound is one of the bot's own outbound
 * messages echoing back. When true, the adapter drops it before routing.
 *
 * Two independent signals:
 *
 *   1. **id-in-cache** — every `sendMessage` return is stashed in
 *      `sentMessageCache` keyed by Baileys' `key.id`. The same id arrives
 *      back on `messages.upsert`; lookup hit ⇒ it's our own echo.
 *
 *   2. **assistant prefix** — when the bot rides on the user's number
 *      (`ASSISTANT_HAS_OWN_NUMBER=false`), every outbound is prefixed
 *      `${ASSISTANT_NAME}: ` before sending (see the deliver path).
 *      Belt-and-suspenders for cache misses after restart or LRU
 *      eviction. When the bot has its own number, the prefix isn't
 *      added on outbound, so this signal is skipped.
 *
 * Non-echo `fromMe` messages flow through to the router unchanged. The
 * router's `engage_mode` decides whether to wake — `mention` only wakes
 * on explicit triggers, `pattern` with `'.'` wakes on everything (the
 * moderator-mode case where the operator is themselves in a group the
 * bot should moderate).
 */
export function isOwnEcho(args: {
  messageId: string | null | undefined;
  text: string;
  sentMessageCache: { has(id: string): boolean };
  assistantHasOwnNumber: boolean;
  assistantName: string;
}): boolean {
  if (args.sentMessageCache.has(args.messageId ?? '')) return true;
  if (!args.assistantHasOwnNumber && args.text.startsWith(`${args.assistantName}:`)) return true;
  return false;
}
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Drop the bot's own outbound echoes universally (see isOwnEcho).
            // Non-echo fromMe messages flow through so moderator-mode wirings
            // (engage_mode='pattern' engage_pattern='.') can wake on the
            // operator's own typing in a group the bot moderates. Mention
            // wirings stay safe because `isMention` is still only true for
            // explicit @-mentions / DMs — non-mention fromMe in a `mention`
            // wiring won't engage at the router.
            if (
              fromMe &&
              isOwnEcho({
